...
def pie_base_velocity_target(
    env: ParkourManagerBasedRLEnv,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Target base linear velocity in the robot base frame."""
    asset: Articulation = env.scene[asset_cfg.name]
    return asset.data.root_lin_vel_b.to(env.device)


def pie_height_scan_target(
    env: ParkourManagerBasedRLEnv,
    sensor_cfg: SceneEntityCfg = SceneEntityCfg("height_scanner"),
) -> torch.Tensor:
    """Target terrain height scan using the same convention as the teacher privileged observation."""
    ray_sensor: RayCaster = env.scene.sensors[sensor_cfg.name]
    return torch.clip(
        ray_sensor.data.pos_w[:, 2].unsqueeze(1) - ray_sensor.data.ray_hits_w[..., 2] - 0.3,
        -1.0,
        1.0,
    ).to(env.device)


def pie_foot_clearance_target(
    env: ParkourManagerBasedRLEnv,
    sensor_names: tuple[str, str, str, str] = (
        "foot_scanner_fl",
        "foot_scanner_fr",
        "foot_scanner_rl",
        "foot_scanner_rr",
    ...
    """Target foot clearance from dedicated downward rays attached to each foot."""
    clearances = []
    for sensor_name in sensor_names:
        ray_sensor: RayCaster = env.scene.sensors[sensor_name]
        foot_height = ray_sensor.data.pos_w[:, 2]
...
def pie_critic_observation(
    ...
    friction = asset.root_physx_view.get_material_properties()[:, 0, 0].unsqueeze(1).to(device)
    stiff_offset = (asset.data.joint_stiffness / asset.data.default_joint_stiffness - 1.0).to(device)
    damp_offset = (asset.data.joint_damping / asset.data.default_joint_damping - 1.0).to(device)
...
class image_features(ManagerTermBase):
    ...
    def __init__(self, cfg: ObservationTermCfg, env: ParkourManagerBasedRLEnv):
        ...
        self.return_history = cfg.params.get("return_history", False)
    ...
    def reset(self, env_ids: Sequence[int] | None = None) -> None:
        ...
        for depth_image, env_id in zip(depth_images, env_ids):
            ...
            self.depth_buffer[env_id] = torch.stack([processed_image] * self.buffer_len, dim=0)
    ...
    def __call__(
        ...
        image_index = -2 if self.buffer_len >= 2 else -1
        return self.depth_buffer[:, image_index].to(env.device)
    ...
    def _normalize_depth_image(self, depth_image):
        depth_image = torch.nan_to_num(
            depth_image,
            nan=self.clipping_range,
            posinf=self.clipping_range,
            neginf=0.0,
        )
        depth_image = torch.clamp(depth_image, 0.0, self.clipping_range)
