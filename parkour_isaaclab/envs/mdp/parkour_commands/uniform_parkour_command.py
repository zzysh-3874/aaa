...
if TYPE_CHECKING:
    ...
    from .parkour_command_cfg import ParkourCommandCfg, PIEVelocityCommandCfg
...
class UniformPIEVelocityCommand(CommandTerm):
    """Direct PIE velocity command [v_x, v_y, omega_yaw] in the base frame."""

    cfg: PIEVelocityCommandCfg

    def __init__(self, cfg: PIEVelocityCommandCfg, env: ParkourManagerBasedEnv):
        super().__init__(cfg, env)
        self.robot: Articulation = env.scene[cfg.asset_name]
        self.vel_command_b = torch.zeros(self.num_envs, 3, device=self.device)
        self.metrics["error_vel_xy"] = torch.zeros(self.num_envs, device=self.device)
    ...
    def __str__(self) -> str:
        msg = "UniformPIEVelocityCommand:\n"
        msg += f"\tCommand dimension: {tuple(self.command.shape[1:])}\n"
        msg += f"\tResampling time: {self.cfg.resampling_time_range}\n"
        msg += f"\tLinear velocity x range: {self.cfg.ranges.lin_vel_x}\n"
        msg += f"\tLinear velocity y range: {self.cfg.ranges.lin_vel_y}\n"
        msg += f"\tYaw velocity range: {self.cfg.ranges.ang_vel_yaw}"
        return msg

    @property
    def command(self) -> torch.Tensor:
        """Desired base velocity command [v_x, v_y, omega_yaw]."""
        return self.vel_command_b
