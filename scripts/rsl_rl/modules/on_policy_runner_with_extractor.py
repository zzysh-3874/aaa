...
from .actor_critic_with_encoder import ActorCriticRMA, SimpleActorCritic
from rsl_rl.utils import store_code_state
from rsl_rl.runners.on_policy_runner import OnPolicyRunner
from .feature_extractors import DefaultEstimator, PIEEstimator
from .ppo_with_extractor import PIEActorInferenceWrapper, PPOWithExtractor 
...
class OnPolicyRunnerWithExtractor(OnPolicyRunner):
    def __init__(self, env: VecEnv, train_cfg: dict, log_dir: str | None = None, device="cpu"):
        ...
        if self.depth_encoder_cfg is None:
            ...
            self.alg.init_storage(
                ...
                [self.policy_cfg.get("num_actor_obs", num_obs)],
                [num_privileged_obs],
                [self.env.num_actions],
            )
            if getattr(self.alg, "uses_pie_estimator", False):
                self.alg.init_pie_estimator_storage(self.env.num_envs, self.num_steps_per_env)
        ...
        for it in range(start_iter, tot_iter):
            ...
            with torch.inference_mode():
                for _ in range(self.num_steps_per_env):
                    pie_estimator_step_input = None
                    if getattr(self.alg, "uses_pie_estimator", False):
                        pie_estimator_step_input = self.alg.cache_pie_estimator_step(obs_dict)
                    # Sample actions
                    actor_obs = self.alg.build_pie_actor_observations(obs, obs_dict)
                    actions = self.alg.act(actor_obs, privileged_obs, hist_encoding)
                    ...
                    if getattr(self.alg, "uses_pie_estimator", False):
                        self.alg.process_pie_estimator_env_step(pie_estimator_step_input, next_obs_dict, dones=dones)
                        self.alg.reset_pie_actor_hidden(dones)
            ...
            if hasattr(self.alg, "set_pie_adasmpl_prob_from_rewards"):
                ...
                # learn_rl uses the parent OnPolicyRunner.log() (no AdaSmpl
                # awareness), so record the prob + episode-reward CV to
                # TensorBoard directly here. These are runtime-only values that
                # are otherwise never persisted.
                _vals = list(rewbuffer)
                if len(_vals) >= 2:
                    _mean = statistics.mean(_vals)
                    _std = statistics.pstdev(_vals)
                    _cv = (_std / abs(_mean)) if abs(_mean) > 1e-6 else 0.0
                else:
                    _cv = 0.0
                _prob = float(getattr(self.alg, "pie_adasmpl_prob", 0.0))
                if self.log_dir is not None and not self.disable_logs and getattr(self, "writer", None) is not None:
                    self.writer.add_scalar("AdaSmpl/prob", _prob, it)
                    self.writer.add_scalar("AdaSmpl/reward_cv", _cv, it)
                if it % 50 == 0:
                    print(f"[AdaSmpl] it={it} prob={_prob:.4f} reward_cv={_cv:.4f}")
    ...
    def get_inference_policy(self, device=None):
        if getattr(self.alg, "use_pie_actor_features", False):
            return self.get_pie_inference_policy(device=device)
    ...
    def get_pie_inference_policy(self, device=None, inference_noise_std=None):
        ...
        if device is not None:
            self.alg.policy.to(device)
            self.alg.estimator.to(device)
        normalizer = self.obs_normalizer if self.cfg["empirical_normalization"] else torch.nn.Identity()
        if device is not None:
